...
from my_depthai_interfaces.msg import HumanLandmarkArray
...
class HumanPoseMarkersNode(Node):
    def __init__(self) -> None:
        super().__init__("human_pose_markers_node")

        self.declare_parameter("landmarks_topic", "/human_pose/landmarks")
        self.declare_parameter("markers_topic", "/human_pose/markers")
    ...
    def _on_landmarks(self, msg: HumanLandmarkArray) -> None:
        ...
        for person_idx, person in enumerate(msg.landmarks):
            base_id = person_idx * 10

            points_marker = Marker()
            points_marker.header = msg.header
            points_marker.ns = "human_points"
            ...
            points_marker.points = [
                Point(x=float(kp.x), y=float(kp.y), z=0.0) for kp in person.landmark
            ]
            marker_array.markers.append(points_marker)

            lines_marker = Marker()
            lines_marker.header = msg.header
            lines_marker.ns = "human_lines"
            ...
            lines_marker.color.g = 0.6
            lines_marker.color.b = 0.2
            lines_marker.color.a = 1.0
            lines_marker.lifetime.sec = lifetime_sec
            lines_marker.lifetime.nanosec = lifetime_nanosec
            for a, b in SKELETON_EDGES:
                if a < len(person.landmark) and b < len(person.landmark):
                    lines_marker.points.append(
                        Point(
                            x=float(person.landmark[a].x),
                            y=float(person.landmark[a].y),
                            z=0.0,
                        )
                    )
                    lines_marker.points.append(
                        Point(
                            x=float(person.landmark[b].x),
                            y=float(person.landmark[b].y),
                            z=0.0,
                        )
                    )
            ...
            text_marker.text = f"{person.label} det:{person.det_score:.2f} lm:{person.lm_score:.2f}"
            text_marker.pose.position.x = float(person.position.x)
            text_marker.pose.position.y = float(person.position.y)
