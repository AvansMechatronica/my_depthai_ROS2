...
def generate_launch_description() -> LaunchDescription:
    ...
    image_topic_arg = DeclareLaunchArgument(
        "image_topic",
        default_value="/animal_pose/image",
        description="ROS topic for annotated animal pose image frames.",
    )
    landmarks_topic_arg = DeclareLaunchArgument(
        "landmarks_topic",
        default_value="/animal_pose/landmarks",
        description="ROS topic for AnimalLandmarkArray detections.",
    )
    markers_topic_arg = DeclareLaunchArgument(
        "markers_topic",
        default_value="/animal_pose/markers",
        description="ROS topic for MarkerArray animal skeleton output.",
    )
    start_markers_arg = DeclareLaunchArgument(
        "start_markers",
        default_value="true",
        description="Start animal pose marker publisher node.",
    )
    ...
    start_rviz_arg = DeclareLaunchArgument(
        "start_rviz",
        default_value="true",
        description="Start RViz2 alongside the animal pose node.",
    )
    rviz_config_arg = DeclareLaunchArgument(
        "rviz_config",
        default_value=PathJoinSubstitution(
            [FindPackageShare("my_depthai_pose_estimation"), "rviz", "animal_pose.rviz"]
        ),
    ...
    animal_pose_node = Node(
        package="my_depthai_pose_estimation",
        executable="animal_pose_node",
        name="animal_pose_node",
        output="screen",
        parameters=[
            {
    ...
    animal_pose_markers_node = Node(
        package="my_depthai_pose_estimation",
        executable="animal_pose_markers_node",
        name="animal_pose_markers_node",
        output="screen",
        parameters=[
            {
                "landmarks_topic": LaunchConfiguration("landmarks_topic"),
