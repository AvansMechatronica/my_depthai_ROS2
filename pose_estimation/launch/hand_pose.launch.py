...
def generate_launch_description() -> LaunchDescription:
    ...
    image_topic_arg = DeclareLaunchArgument(
        "image_topic",
        default_value="/hand_pose/image",
        description="ROS topic for annotated hand pose image frames.",
    )
    landmarks_topic_arg = DeclareLaunchArgument(
        "landmarks_topic",
        default_value="/hand_pose/landmarks",
        description="ROS topic for HandLandmarkArray detections.",
    )
    markers_topic_arg = DeclareLaunchArgument(
        "markers_topic",
        default_value="/hand_pose/markers",
        description="ROS topic for RViz MarkerArray output.",
    )
    ...
    fastdds_builtin_transports_arg = DeclareLaunchArgument(
        "fastdds_builtin_transports",
        default_value="UDPv4",
        description="Fast DDS builtin transports. Use UDPv4 to avoid SHM lock errors.",
    )
    ...
    start_rviz_arg = DeclareLaunchArgument(
        "start_rviz",
        default_value="true",
        description="Start RViz2 alongside the hand pose node.",
    )
    ...
    set_fastdds_transport = SetEnvironmentVariable(
        ...
        value=LaunchConfiguration("fastdds_builtin_transports"),
    )
    ...
    return LaunchDescription(
        [
            ...
            fastdds_builtin_transports_arg,
            start_urdf_arg,
            camera_model_arg,
            tf_prefix_arg,
            base_frame_arg,
